 ...
 */
public class DrawTextPanelMine extends JPanel  {
	...
	private class Canvas extends JPanel {
		...
		protected void paintComponent(Graphics g) {
			...
			if (theString != null) {
				...
				while (iter.hasNext()) {
					DrawTextItem text = iter.next(); // items from the ArrayList()
					text.draw(g);
				}
...
//		s.setBackgroundTransparency(0.7);  // Default is 0, meaning background is not transparent.
		...
		theString.add(s); // Draw multiple text values
//		theString = s;  // Set this string as the ONLY string to be drawn on the canvas!
	...
	public JMenuBar getMenuBar() {
		if (menuBar == null) {
			...
			JMenuItem randomKey = new JMenuItem("Random text College....");
			randomKey.setAccelerator(KeyStroke.getKeyStroke(commandKey + "R"));
			randomKey.addActionListener(menuHandler);
			fileMenu.add(randomKey);
	...
	private void doMenuCommand(String command) {
		if (command.equals("Save...")) { // save all the string info to a file
			File textOutput = fileChooser.getOutputFile(this, "Select a File Name", "textimage.txt");
			try {
				PrintWriter file = new PrintWriter(textOutput);
				Color c = canvas.getBackground();
				file.println(c.getRGB());
				for (DrawTextItem s : theString) {
					file.println(s.getString());
					file.println(s.getX());
					file.println(s.getY());
					Color textColor = s.getTextColor();
					file.println(textColor.getRGB());
				}
				file.close();
			} catch (FileNotFoundException e) {
				JOptionPane.showMessageDialog(this, "Sorry, the Save command is having trouble to save." + e);
			}
//			
		}
		else if (command.equals("Open...")) { // read a previously saved file, and reconstruct the list of strings
			File openInput = fileChooser.getOutputFile(this, "Select a File Name");
			try {
				Scanner scanner = new Scanner (openInput);
				theString.clear();
				canvas.repaint();
				...
				while (scanner.hasNext()) {
					...
					DrawTextItem s = new DrawTextItem(fileString);
					s.setX(Integer.parseInt(scanner.next()));
					scanner.nextLine();
					s.setY(Integer.parseInt(scanner.next()));
					scanner.nextLine();
					Color textC = Color.decode(scanner.next());
				...
				JOptionPane.showMessageDialog(this, "Sorry, Something went wrong with open file." + e);
			}
		}
		
		else if (command.equals("Random text College....")) {
			File randomInput = fileChooser.getInputFile();
			try {
				Scanner scanner = new Scanner(randomInput);//new scanner to read the file
				theString.clear();
